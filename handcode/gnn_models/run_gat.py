...
def gat_run(dataname="cora", random_seed=2020, train_epoch=100, lr=0.2, dropout=0.5):
    ...
    gat = gg.gallery.GAT(data.graph, device="cuda:0").process().build(dropout=dropout, lr=lr)
    gat.train(tr_ind, val_ind, epochs=train_epoch)
    result = gat.test(ts_ind)
...
def model_startup(dataname, runs, **model_params):
    ...
    conf.update(allconfs['gat'][dataname])
    conf.update(model_params)

    scores = []
    for run in range(runs):
        score = gat_run(dataname = dataname,
...
def search_params(dataname, log_dir):
    ...
    msg = "gat; %s; best_score, std, best_params %s %s %s\n" % (dataname, best_score, std, best_params)
    print(msg)
    sharedutils.add_log(os.path.join(log_dir, "gat_search.log"), msg)
...
if __name__ == '__main__':
    ...
    if do_search_params == "yes":
        allconfs, conf = sharedutils.read_config("./models_conf.json"), {}
        search_params(dataname, allconfs['gat']['log_dir'])
    else:
