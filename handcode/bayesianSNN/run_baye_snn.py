...
from bayesian_model_lif_fc import model_lif_fc
...
def snn_run(dataname, **new_conf):
  ...
  data = data_fixed
  num_per_class = cnf['num_per_class']
  if cnf['ggp_embedding'] == True: 
    print("Using GGP's embedding!")
    mat, tag = rd.ggp_embedding(num_per_class,data_fixed) 
  else: mat, tag = rd.conv_graph(data_fixed)
  ...
  train_data_loader, val_data_loader, test_data_loader = rd.fixed_numpy2dataloader(num_per_class, data_fixed,tr_val_mat, ts_mat,
  ...
  print("train, valiadation,test's shape:", tr_val_mat.shape, ts_mat.shape)
...
if __name__ == '__main__':
  ...
  do_search_params, dataname, runs = "no", "cora", 10
